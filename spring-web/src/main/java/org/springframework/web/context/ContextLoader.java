...
public class ContextLoader {
	...
	/**
	 * 为给定的servlet上下文（Servlet Context）初始化Spring的Web应用程序上下文 WebApplicationContext
	 *
	 *
	 * 思路：首先 Spring 会检测 ServletContext 中 ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE 属性有没有被设置，
	...
	public WebApplicationContext initWebApplicationContext(ServletContext servletContext) {
		// <1> 如果 ServletContext 中 ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE 属性值
		...
		try {
			...
			// <9> 当发生异常，记录异常到 WebApplicationContext.ROOT_WEB_APPLICATION_CONTEXT_ATTRIBUTE 中，
			// 不再重新初始化。
	...
	//该方法首先会调用 determineContextClass 判断创建什么类型的容器，
	// 默认为 XmlWebApplicationContext。然后调用 instantiateClass
	// 方法通过反射的方式创建容器实例

	//创建WebApplicationContext对象（业务容器 dao service...）
	...
	* */
	protected void configureAndRefreshWebApplicationContext(ConfigurableWebApplicationContext wac, ServletContext sc) {
		...
		if (ObjectUtils.identityToString(wac).equals(wac.getId())) {
			...
			// 情况一，使用 contextId 属性 ; 从 ServletContext 中获取用户配置的 contextId 属性
		...
		if (configLocationParam != null) {
			//并将contextConfigLocation 配置设置到容器
